...
class TestResolvePreToolBlock:
    ...
    def test_approve_message_reaches_the_human_verbatim(self, monkeypatch):
        """The approver must see WHO a write lands on, in full.

        An approval plugin puts the affected targets in the directive's
        ``message`` — the evaos-pipedream-connect contract renders them as
        "affected targets (N, shown in full)" and deliberately never clips a
        target value, because a truncated recipient is one an approver can
        misread as the intended one. The host forwards that message to the
        gate as the approval ``reason`` unchanged: no clipping, no summary, no
        redaction pass between the plugin and the human. This is the property
        the approval path's safety rests on, so it is pinned here rather than
        left implicit in the pass-through.
        """
        from hermes_cli.plugins import resolve_pre_tool_block

        message = (
            "Outbound send via mattermost.\n"
            "affected targets (3, shown in full):\n"
            "  channel: town-square\n"
        ...
        monkeypatch.setattr(
            "hermes_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
                {"action": "approve", "message": message,
        ...
        assert seen["reason"] == message
        # Spelled out, so a future "shorten the prompt" change fails loudly
        # on the values rather than quietly on an equality check.
        for target in (
            "town-square",
            "finance-team@example.com",
            "incident-response@example.com",
        ):
            assert target in seen["reason"]
        assert "…" not in seen["reason"] and "..." not in seen["reason"]
