...
Host source: `40ed52e69aa02e00ddf271513784d1d7eee29dad`
...
| Tool transcript ordering | Paired assistant tool-call row, pre-effect database flush, canonical tool result row, duplicate-id cache/conflict check | Request id, name, and arguments | No tool effect occurs before its paired assistant row is durable; same-id/same-payload retries are idempotent on the binding. |
