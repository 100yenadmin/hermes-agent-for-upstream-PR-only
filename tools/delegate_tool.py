...
def _build_child_agent(
    ...
    override_api_key_set: bool = False,
    ...
    # Explicit routes pin credential ownership too. In particular, a trusted
    # OAuth/external/virtual/keyless runtime may intentionally have no string
    # API key; never replace that explicit empty value with the parent's key.
    effective_api_key = (
        override_api_key if override_api_key_set else (override_api_key or parent_api_key)
    )
...
_TRUSTED_ROUTE_AUTH_TYPES = frozenset(
    {"oauth_device_code", "oauth_external", "oauth_minimax", "external_process", "copilot", "virtual", "aws_sdk", "vertex"}
)
...
def _validate_delegation_routes(cfg: dict) -> Dict[str, Dict[str, str]]:
    ...
    for raw_alias, entry in raw_routes.items():
        alias = raw_alias.strip() if isinstance(raw_alias, str) else ""
        if (
            not alias
            or not _SAFE_ROUTE_NAME_RE.fullmatch(alias)
            or alias.lower().startswith(_SECRET_LIKE_PREFIXES)
        ):
            raise ValueError("delegation.routes contains an invalid route alias")
        if alias in routes or not isinstance(entry, dict) or set(entry) != _ROUTE_FIELDS:
            raise ValueError(
...
def _route_meter_policy(cfg: dict, credentials: Dict[str, Any]) -> str:
    """Return a runtime billing mode, failing closed when it is unsafe."""
    ...
    keyless = bool(getattr(provider_def, "keyless", False))
    if not auth_type or auth_type in {"unknown", "none"}:
        raise ValueError("Delegation route has unknown provider authentication metadata")

    try:
        billing = resolve_billing_route(
            model.strip(), provider=provider.strip(), base_url=credentials.get("base_url")
        )
    except Exception:
        billing = None
    billing_mode = str(getattr(billing, "billing_mode", "") or "").strip()
    if keyless:
        # Keyless is an explicit provider contract, not an inference from an
        # empty key or endpoint. Its safe receipt is deliberately stable even
        # when the generic pricing classifier has no model entry.
        return "non_metered"
    ...
    if billing_mode == "subscription_included":
        if auth_type not in _TRUSTED_ROUTE_AUTH_TYPES:
            raise ValueError("Delegation route has untrusted subscription metadata")
        return billing_mode
...
def _resolve_task_routes(
    ...
    """Resolve and policy-check every task route before child construction."""
    ...
    if not explicit:
        ...
        return [
            {"explicit": False, "route": None, "credentials": credentials}
            for _ in task_list
        ], False
...
def _attach_route_receipt(entry: Dict[str, Any], route_info: Dict[str, Any], child: Any) -> None:
    """Attach only safe, runtime-derived metadata for an explicit route."""
    if not route_info.get("explicit"):
        return
    credentials = route_info.get("credentials") or {}
    provider = getattr(child, "provider", None) or credentials.get("provider")
    model = getattr(child, "model", None) or credentials.get("model")
    if (
        ...
        provider = credentials.get("provider") or "unknown"
    if (
        ...
        model = credentials.get("model") or "unknown"
    if credentials.get("keyless"):
        billing_mode = route_info.get("billing_mode", "non_metered")
        ...
        try:
            ...
            billing_mode = resolve_billing_route(
                model, provider=provider, base_url=getattr(child, "base_url", None)
            ).billing_mode
        except Exception:
...
def delegate_task(
    ...
    # Validate the complete route map and resolve every task's credentials and
    # billing policy before creating the first child or consuming model usage.
    try:
        task_routes, explicit_routes = _resolve_task_routes(
            task_list, cfg, parent_agent, credentials_cfg=credentials_cfg
        )
    ...
    for i, t in enumerate(task_list):
        ...
        try:
            child = _build_child_preserving_parent_tools(
                ...
                override_api_key_set=bool(task_routes[i].get("explicit")),
    ...
    def _execute_and_aggregate(*, honor_parent_interrupt: bool = True) -> dict:
            ...
            for entry in results:
                ...
                if isinstance(task_index, int) and 0 <= task_index < len(task_routes):
                    _attach_route_receipt(
                        entry, task_routes[task_index], child_by_index.get(task_index)
                    )
        ...
        if explicit_routes:
            route_keys = {
                (entry.get("route"), entry.get("provider"), entry.get("model"))
                for entry in results
            }
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    auth_type = str(runtime.get("auth_type") or "").strip().lower()
    keyless = bool(runtime.get("keyless", False))
    if not api_key and not keyless and auth_type not in _TRUSTED_ROUTE_AUTH_TYPES:
        raise ValueError(
